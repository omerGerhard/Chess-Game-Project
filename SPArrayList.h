...
typedef struct sp_array_list_cell_t {
	SPCell source;
	SPCell target;
	char tool;
	char eatenTool;
...
/**
 * creates a new spArrayCell based on -
 * source SPCell - created from 'srcRow' and 'srcCol'
 * target SPCell - created from 'dstRow' and 'dstCol'
 * tool - the tool which moves from the source cell of the board to the target cell
 * eaten tool - the captured tool from the target cell (if exist), otherwise '_'
 */
...
/**
 * Frees all memory resources associated with the source array list.
...
/**
 * Inserts element 'elem' at the end of the list 'src'.
 * @param src   - the source array list
...
/**
 * Returns the element located at the end of the list 'src'.
 * @param src - the source array list
 * @return
